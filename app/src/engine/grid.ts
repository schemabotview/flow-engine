...
// Recursive grid resolver. Emits top-left pixel boxes (the corner React Flow positions a
// node by). A node with `children` + `layout` resolves those children INSIDE its own box,
// so a container truly measures its contents. Pure: id → absolute Box, no React.
...
export function resolveGrid(
  ...
  // `?.` so the module is safe to run outside Vite (import.meta.env is undefined there).
